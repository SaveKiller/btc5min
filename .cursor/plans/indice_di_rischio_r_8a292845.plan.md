...
- `max(Rq, Rz)` può esistere soltanto come benchmark prudenziale `Rmax`, non come probabilità.
...
- Tie o fascia neutra attorno a 50c: target economicamente ambiguo; `Rq=-`, `Rz=-`, ingresso non eleggibile.
- Chainlink stale sulla riga o dentro la finestra: `Rz=-`; `Rq` può restare disponibile se il CLOB è completo.
- Storia insufficiente: `Rz=-` con motivo `insufficient_history`.
- Volatilità nulla/quasi nulla: `Rz=-` con motivo `zero_vol`.
...
```text
... V30=18 V60=22 V120=31  Rq=5 Rz=4 eligible=full
... V30=--- V60=--- V120=---  Rq=5 Rz=- eligible=q_only(stale)
... V30=18 V60=22 V120=31  Rq=- Rz=- eligible=no(partial)
```

`Rz` nel TXT usa la finestra primaria W60; tutte le `Pz_W` rimangono disponibili nel report. Le probabilità continue restano nell'output JSON/CSV di valutazione. `Rc` non viene mostrata finché non è validata.
...
9. Integrare inizialmente `Rq`, `Rz` W60 ed eleggibilità in `convert`.
