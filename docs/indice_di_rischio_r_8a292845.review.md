...
Il piano ha una buona struttura da POC: separa un segnale statistico (`Rs`) da uno di mercato (`Rq`), mantiene il calcolo live-safe, lascia invariato il `.bin` e rimanda correttamente la calibrazione a un archivio più grande.
...
3. la preferenza dichiarata per `Rc = max(Rs, Rq)` non è ancora giustificata e la preview indipendente svolta in questa revisione dà anzi un primo segnale contrario;
...
`max(Rs, Rq)` è comprensibile come allarme prudenziale, ma non come combinazione probabilistica:
...
- mantenere `Rs` e `Rq` separati nella fase corrente;
...
Il confronto decisivo non è «Rc è migliore di Rs», ma «aggiungere le feature fisiche migliora davvero il baseline di mercato su giornate mai viste?».
...
- se utile, mostrare `Rs` sui partial solo come diagnostica statistica, ma non usarlo come fallback di `Rc` per autorizzare una puntata;
...
Per `Rs` va inoltre usato il valore float interno, non il `VW` intero arrotondato nel TXT:
...
La scelta finale deve premiare la finestra o combinazione che migliora **out-of-sample il modello basato sulla quota**, non quella che ottiene il miglior risultato stand-alone su Rs.
...
### Rs — mantenere come baseline statistica
...
La firma proposta `compute_risk_indices(ticks, vols)` non contiene il PTB necessario a Rs e non lascia spazio alle feature del book. Un contratto più esplicito dovrebbe ricevere almeno `ptb_used`, tick e volatilità; i book possono restare opzionali finché non entrano in Rq.
...
2. `Rs` come diagnostica statistica multi-finestra;
