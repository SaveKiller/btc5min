...
def format_table_row(sec: str, time: str, quote: str, delta: str, gain_val: str, btc_val: str,
    ...
    return (
        ...
        f"{delta:>5}  {gain_val:>7}  {btc_val:>8}  {vol_tokens:<{vol_w}}  {risk_tokens:<{risk_w}}"
    )
...
def format_column_header() -> str:
    ...
    return (
        f"{'sec':>3}  {'time':>5}  {'quote':<9}  "
        f"{'delta':>5}  {'gain%':>7}  {'btc':>8}  {vol_hdr}  {risk_hdr}"
    )
...
def format_data_row_partial(sec: int, side: str, chainlink: float, ptb: float, stale: bool,
    ...
    return format_table_row(
        ...
        format_delta_cell(chainlink, ptb, stale), "  ---", format_btc_cell(chainlink), vol_tokens, risk_tokens,
    )
...
def format_data_row(sec: int, up_prob: int, down_prob: int, chainlink: float, ptb: float, gain: float,
    ...
    return format_table_row(
        ...
        format_delta_cell(chainlink, ptb, stale), format_gain_pct(gain), format_btc_cell(chainlink), vol_tokens, risk_tokens,
    )


def render_round_txt(header: dict, ticks: np.ndarray, warnings: list[str]) -> str:
