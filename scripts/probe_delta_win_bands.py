"""Probe win rate osservato per finestra |delta| ±2 sui round reali (supporto metodo A)."""
...
def _window_table(samples: list[dict]) -> list[dict]:
    groups: dict[tuple[int, str], list[dict]] = defaultdict(list)
    for s in samples:
        groups[(s["sec"], s["delta_window"])].append(s)
    rows = []
...
def main() -> None:
    ...
    print(f"written {out}")
    print(f"windows={len(table)} sec90_spread={spread90 * 100:.1f}pp")
